...
import { buildStudentWrongQuestionsCsv, teacherReportRepository } from "@/server/repositories/teacher-report-repository";
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const csv = buildStudentWrongQuestionsCsv(detail);
  return new Response(`\uFEFF${csv}`, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="chem2exam-wrong-questions-${Date.now()}.csv"`
    }
  });
}
