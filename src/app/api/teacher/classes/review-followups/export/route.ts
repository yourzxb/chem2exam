...
import { buildTeacherReviewFollowupCsv, teacherReportRepository } from "@/server/repositories/teacher-report-repository";
...
export async function GET(request: Request) {
  ...
  const csv = buildTeacherReviewFollowupCsv(report);

  return new Response(`\uFEFF${csv}`, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="chem2exam-review-followups-${Date.now()}.csv"`
    }
  });
}
